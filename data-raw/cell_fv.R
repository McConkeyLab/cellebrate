## code to prepare `cell_fv` dataset goes here
library(httr2)
library(tidyverse)
library(fs)
...
fv_locs <- download_file(
  linked_cell_analyses$links |> lapply(`[[`, "filtered_variants") |> unlist(),
  path(tempdir(),
...
fv_extracted_locs <- mapply(extract_variant_file, fv_locs, linked_cell_analyses$samples,
                             tempdir())
...
fv <- mapply(read_variant_call_file,
              fv_extracted_locs,
              "gbci62",
              linked_cell_analyses$name,
...
fv <- lapply(fv, \(x) {
  x$FATHMM <- as.character(x$FATHMM)
  x$omim <- as.character(x$omim)
  x
})
...
cell_fv <- filter_gbci_genes(cell_fv)
...
cell_fv <- cell_fv |> select(-workflow, -MyVariantDefaultDb_hg19, -exac, -polyphen,
                   -go_20200511, -hg19_exac_1, -hg19_cosmic_82, -hg19_dbsnp_154,
                   -hg19_omim_20191001, -hg19_refgene_201, -som_pval, -sift)

usethis::use_data(cell_fv, overwrite = TRUE)
